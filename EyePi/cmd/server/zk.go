...
func notifyZkOfMetricsIfNeeded(logger zerolog.Logger) []zkCancelFunc {
	...
	logger.Info().Str("service", "EyePi").Msg("announcing metrics endpoint to zookeeper")
	...
	logger.Info().Str("service", "EyePi").Msg("Service successfully registered metrics endpoint to zookeeper")
...
func notifyZkOfRPCServerIfNeeded(logger zerolog.Logger) []zkCancelFunc {
	...
	logger.Info().Str("service", "EyePi").Msg("announcing rpc endpoint to zookeeper")
	cancel := gk.Announce(viper.GetStringSlice("zk_connection_string"), &gk.Registration{
	...
	logger.Info().Str("service", "EyePi").Msg("Service successfully registered rpc endpoint to zookeeper")
...
func notifyZkOfGatewayEndpointIfNeeded(logger zerolog.Logger) []zkCancelFunc {
	...
	logger.Info().Str("service", "EyePi").Msg("announcing gateway endpoint to zookeeper")
